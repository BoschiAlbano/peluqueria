...
async function crearDueno(
  ...
  if (error || !data.user) {
    console.warn(
      `✗ No se pudo crear el login del dueño: ${error?.message ?? "error desconocido"}`,
    );
    return;
  }
  ...
  console.log(
    `✓ Dueño creado: usuario "${username}", contraseña "${password}".`,
  );
}
...
async function main() {
  ...
  console.log(
    "Sembrando configuración de comisión (60% peluquero / 40% dueño)...",
  );
